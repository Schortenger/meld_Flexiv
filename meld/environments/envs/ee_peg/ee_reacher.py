...
class EEReachingEnv(mujoco_env.MujocoEnv):
    ...
    def __init__(self, xml_path=None, goal_site_name=None, action_mode='joint_position'):
        ...
        # else:
        # self.start_positions.append(np.array([0.525, 0, 0.2, 0]))
        self.start_qpos = np.array([0.525, 0, 0.2, 0])


        # vars
        self.action_mode = action_mode
        self.num_joint_dof = 4
        self.frame_skip = 100
        if xml_path is None:
            xml_path = os.path.join(SCRIPT_DIR, 'assets/simpleEE_4box.xml')
    ...
    def _get_FT(self):
        """
            self.sim.data.sensordata[0:3]:[Tx,Ty,Tz] for joint ;
            self.sim.data.sensordata[3:6]:[Fx,Fy,Fz] for joint;
        """
    ...
    def reset_model(self):
        ...
        for i in range(len(self.sim.data.qpos)):
            self.sim.data.qpos[i] = self.start_qpos[i]
        return self.get_obs()
...
class SawyerReachingEnvMultitask(EEReachingEnv):
